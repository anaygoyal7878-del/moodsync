...
function toNormalized(row: BiometricReadingRow): NormalizedBiometricReading {
  return {
    ...
    deepSleepMinutes: row.deepSleepMinutes ?? undefined,
    remSleepMinutes: row.remSleepMinutes ?? undefined,
    lightSleepMinutes: row.lightSleepMinutes ?? undefined,
  ...
  async bulkInsert(readings: NormalizedBiometricReading[]): Promise<number> {
    ...
    const result = await prisma.biometricReading.createMany({
      data: readings.map((r) => ({
        ...
        deepSleepMinutes: r.deepSleepMinutes ?? null,
        remSleepMinutes: r.remSleepMinutes ?? null,
        lightSleepMinutes: r.lightSleepMinutes ?? null,
