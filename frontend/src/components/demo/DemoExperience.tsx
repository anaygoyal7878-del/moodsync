"use client";

import { useEffect, useRef, useState } from "react";
import { Button } from "@/components/ui/Button";

interface Step {
  caption: string;
  recovery: number;
  ruleMatched: boolean;
  lightOn: boolean;
  warm: boolean;
  brightness: number;
  playlist: string | null;
  logged: boolean;
}

/**
 * A single, real MoodSync scenario (recovery drop -> Hue + Spotify react
 * -> logged), not a menu of unrelated feature demos — this app connects
 * wearables to Hue/Spotify specifically, so the walkthrough only shows
 * what the product actually does today.
 */
const STEPS: Step[] = [
  {
    caption: "Your WHOOP syncs this morning.",
    recovery: 78,
    ruleMatched: false,
    lightOn: true,
    warm: false,
    brightness: 100,
    playlist: null,
    logged: false,
  },
  {
    caption: "Recovery comes back low — 32%.",
    recovery: 32,
    ruleMatched: false,
    lightOn: true,
    warm: false,
    brightness: 100,
    playlist: null,
    logged: false,
  },
  {
    caption: 'MoodSync matches your rule: "Wind down when recovery is low."',
    recovery: 32,
    ruleMatched: true,
    lightOn: true,
    warm: false,
    brightness: 100,
    playlist: null,
    logged: false,
  },
  {
    caption: "Your Hue lights dim and warm automatically.",
    recovery: 32,
    ruleMatched: true,
    lightOn: true,
    warm: true,
    brightness: 30,
    playlist: null,
    logged: false,
  },
  {
    caption: "Spotify switches to a wind-down playlist.",
    recovery: 32,
    ruleMatched: true,
    lightOn: true,
    warm: true,
    brightness: 30,
    playlist: "Wind Down",
    logged: false,
  },
  {
    caption: "Logged to your automation history — what fired, and why.",
    recovery: 32,
    ruleMatched: true,
    lightOn: true,
    warm: true,
    brightness: 30,
    playlist: "Wind Down",
    logged: true,
  },
];

const STEP_DURATION_MS = 3200;

export function DemoExperience() {
  const [index, setIndex] = useState(0);
  const [autoPlay, setAutoPlay] = useState(true);
  const reducedMotionRef = useRef(false);

  useEffect(() => {
    reducedMotionRef.current = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    if (reducedMotionRef.current) setAutoPlay(false);
  }, []);

  useEffect(() => {
    if (!autoPlay) return;
    const timer = setInterval(() => setIndex((i) => (i + 1) % STEPS.length), STEP_DURATION_MS);
    return () => clearInterval(timer);
  }, [autoPlay]);

  const step = STEPS[index]!;

  return (
    <div className="flex flex-col gap-6">
      <div className="grid gap-6 sm:grid-cols-2">
        {/* Biometric card */}
        <div className="rounded-2xl border border-line bg-surface p-6">
          <p className="text-xs uppercase tracking-wide text-ink-muted">Recovery (WHOOP)</p>
          <p className="mt-2 text-4xl font-semibold tabular-nums transition-colors duration-700" style={{ color: step.recovery < 50 ? "#ff9075" : "#f5f1ec" }}>
            {step.recovery}%
          </p>
          <div className="mt-3 h-1.5 w-full overflow-hidden rounded-full bg-surface-raised">
            <div
              className="h-full rounded-full transition-all duration-700"
              style={{ width: `${step.recovery}%`, backgroundColor: step.recovery < 50 ? "#ff7a59" : "#726a80" }}
            />
          </div>
          <p
            className={`mt-4 rounded-lg border px-3 py-2 text-xs transition-all duration-500 ${
              step.ruleMatched ? "border-brand/40 bg-brand/10 text-ink" : "border-line text-ink-muted opacity-60"
            }`}
          >
            Rule: Wind down when recovery is low {step.ruleMatched && "· matched"}
          </p>
        </div>

        {/* Home reaction card */}
        <div className="rounded-2xl border border-line bg-surface p-6">
          <p className="text-xs uppercase tracking-wide text-ink-muted">Your home</p>
          <div className="mt-3 flex items-center gap-4">
            <div
              className="h-12 w-12 shrink-0 rounded-full transition-all duration-700"
              style={{
                backgroundColor: step.warm ? "#ffb37a" : "#f5f1ec",
                opacity: step.brightness / 100,
                boxShadow: step.lightOn ? `0 0 ${step.brightness / 2.5}px ${step.brightness / 6}px ${step.warm ? "#ffb37a55" : "#f5f1ec55"}` : "none",
              }}
              aria-hidden="true"
            />
            <div>
              <p className="text-sm font-medium">Living Room Lamp</p>
              <p className="text-xs text-ink-muted">
                {step.warm ? `Warm · ${step.brightness}% brightness` : `Bright · ${step.brightness}% brightness`}
              </p>
            </div>
          </div>

          <div
            className={`mt-4 flex items-center gap-3 rounded-lg border px-3 py-2 transition-all duration-500 ${
              step.playlist ? "border-line-strong bg-surface-raised opacity-100" : "border-line opacity-40"
            }`}
          >
            <span className="h-2 w-2 shrink-0 rounded-full bg-brand" aria-hidden="true" />
            <div>
              <p className="text-xs text-ink-muted">Spotify</p>
              <p className="text-sm font-medium">{step.playlist ?? "No playback yet"}</p>
            </div>
          </div>
        </div>
      </div>

      {/* Automation log */}
      <div
        className={`rounded-2xl border px-4 py-3 text-sm transition-all duration-500 ${
          step.logged ? "border-brand/30 bg-brand/5 text-ink opacity-100" : "border-line text-ink-muted opacity-50"
        }`}
      >
        {step.logged ? (
          <span>
            <span className="font-medium">Wind down when recovery is low</span> · Executed · just now
          </span>
        ) : (
          "Automation history entry will appear here once a rule fires."
        )}
      </div>

      {/* Caption + controls */}
      <div className="flex flex-col items-center gap-4 text-center">
        <p className="min-h-6 text-sm text-ink-secondary">{step.caption}</p>

        <div className="flex items-center gap-1.5" role="tablist" aria-label="Demo steps">
          {STEPS.map((s, i) => (
            <button
              key={s.caption}
              role="tab"
              aria-selected={i === index}
              aria-label={`Step ${i + 1}`}
              onClick={() => {
                setIndex(i);
                setAutoPlay(false);
              }}
              className={`h-1.5 rounded-full transition-all duration-300 ${i === index ? "w-6 bg-brand" : "w-1.5 bg-line-strong hover:bg-ink-muted"}`}
            />
          ))}
        </div>

        <div className="flex items-center gap-2">
          <Button
            variant="ghost"
            onClick={() => {
              setIndex((i) => (i - 1 + STEPS.length) % STEPS.length);
              setAutoPlay(false);
            }}
          >
            Back
          </Button>
          <Button variant="secondary" onClick={() => setAutoPlay((p) => !p)}>
            {autoPlay ? "Pause" : "Play"}
          </Button>
          <Button
            variant="ghost"
            onClick={() => {
              setIndex((i) => (i + 1) % STEPS.length);
              setAutoPlay(false);
            }}
          >
            Next
          </Button>
        </div>
      </div>
    </div>
  );
}
