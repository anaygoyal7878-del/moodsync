...
/** All the tunable props read fresh every animation frame from a ref
 * (not React state) — see the `propsRef` assignment below — so changing
 * a prop never tears down and rebuilds the WebGL context; only the
 * mount/unmount effect (deps: []) does that. */
export default function Strands({
  ...
  // Kept out of the render body (a ref write during render is disallowed
  // by React's rules-of-hooks lint) — a layout effect still runs before
  // the browser paints, so the rAF loop below never reads a stale frame
  // of props.
  useLayoutEffect(() => {
    propsRef.current = {
      colors,
