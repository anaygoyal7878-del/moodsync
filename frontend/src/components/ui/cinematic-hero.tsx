...
/** Ported from a third-party "cinematic hero" component reference.
 * Two real adaptations from the original: (1) every `--color-background`/
 * `--color-foreground`/`bg-background`/`text-foreground`/
 * `text-muted-foreground` reference is remapped to this project's real
 * design tokens (`--color-canvas`/`--color-ink`/`--color-ink-muted` —
 * see globals.css's `@theme inline` block) since the shadcn defaults
 * this component assumed don't exist here and would resolve to nothing.
 * (2) the original's two fake "Download on the App Store"/"Get it on
 * Google Play" badges are replaced with a single real CTA to /signup —
 * MoodSync has no published mobile app, so store badges linking nowhere
 * would be dishonest. */
const INJECTED_STYLES = `
  ...
  .text-3d-matte {
      ...
      text-shadow:
          ...
          0 2px 4px color-mix(in srgb, var(--color-ink) 10%, transparent);
  }

  .text-silver-matte {
      background: linear-gradient(180deg, var(--color-ink) 0%, color-mix(in srgb, var(--color-ink) 40%, transparent) 100%);
      -webkit-background-clip: text;
      -webkit-text-fill-color: transparent;
      background-clip: text;
      transform: translateZ(0); /* Hardware acceleration to prevent WebKit clipping bug */
      filter:
          drop-shadow(0px 10px 20px color-mix(in srgb, var(--color-ink) 15%, transparent))
          drop-shadow(0px 2px 4px color-mix(in srgb, var(--color-ink) 10%, transparent));
  }
...
export function CinematicHero({
  ...
  cardDescription = (
    <>
      <span className="text-white font-semibold">MoodSync</span> reads your real biometric signals — heart
      rate, recovery, sleep, stress — and automatically adapts the smart home devices you already own.
      No new hardware required.
    </>
  ),
  metricValue = 82,
  metricLabel = "Wellness Score",
  ctaHeading = "Start free during beta.",
  ctaDescription = "Connect a wearable and a smart home device in minutes, and let MoodSync handle the rest.",
  className,
  ...props
}: CinematicHeroProps) {
  ...
  return (
    <div
      ref={containerRef}
      className={cn("relative w-screen h-screen overflow-hidden flex items-center justify-center bg-canvas text-ink font-sans antialiased", className)}
    ...
    >
      ...
      {/* BACKGROUND LAYER 2: Real CTA — replaces the original component's
       * fake App Store / Google Play badges (see this file's top doc
       * comment for why). */}
      ...
      <div className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none" style={{ perspective: "1500px" }}>
        ...
        >
          ...
          <div className="relative w-full h-full max-w-7xl mx-auto px-4 lg:px-12 flex flex-col justify-evenly lg:grid lg:grid-cols-2 items-center lg:gap-8 z-10 py-6 lg:py-0">
                    ...
                    <div className="relative w-full h-full pt-12 px-5 pb-8 flex flex-col">
                      <div className="phone-widget flex justify-between items-center mb-8">
                        <div className="flex flex-col">
                          ...
                          <span className="text-xl font-bold tracking-tight text-white drop-shadow-md">MoodSync</span>
                        </div>
            ...
            {/* 3. BOTTOM (Mobile) / LEFT (Desktop): CARD DESCRIPTION TEXT */}
